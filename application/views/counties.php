...
<html>
  ...
  <body class="fixed-header ">
        ...
        <div class="content ">
          ...
          <div class="container-fluid container-fixed-lg m-t-10">
            ...
            <div class="row">
              ...
              <button class="btn btn-primary pull-right addNewCounty" data-toggle="modal" data-target="#addNewCountyModal">Yeni İlçe Ekle</button>
              </div>
              <div class="col-md-4 pull-left" style="padding-left: 0px">
                <div class="col-xs-12">
                <input type="text" id="search-table" class="form-control pull-left" placeholder="İlçe Ara">
                </div>
              </div>
            </div>
            <div class="row">
              ...
              <tbody>
                ...
                foreach ($counties as $key => $county) {
                  echo '<tr id="'.$county->CountyID.'">
                    <td>'.$county->CountyName.'</td>
                    <td>'.$county->CityName.'</td>
                    <td><div class="pull-left"><button class="btn btn-warning getCountyDetails btn-xs m-r-10" id="duzenle">Düzenle</button><button class="btn btn-danger deleteCountyModal btn-xs">Sil</button></div></td>
                  </tr>';
                }
                ?>
              </tbody>
        ...
        <div class="modal-content-wrapper">
          <div class="modal-content">
            <div class="modal-header clearfix text-left">
              ...
              <h5>İlçe <span class="semi-bold">Bilgileri</span></h5>
              <p class="p-b-10">Aşağıda ilçe ile ilgili bilgileri bulabilirsiniz.</p>
            </div>
            <div class="modal-body">
              <form role="form" id="updateCountyData">
                <div class="form-group appendCountyDataHere">
                </div>
                <button type="submit" class="btn btn-primary m-t-5 updateCounty">Bilgileri Düzenle</button>
                <div class="alert alert-danger updateModalError unvisible m-t-10"></div>
              </form>
            </div>
          </div>
        </div>
    ...
    <div class="modal fade stick-up disable-scroll" id="addNewCountyModal" role="dialog" aria-hidden="false">
      <div class="modal-dialog ">
        <div class="modal-content-wrapper">
          <div class="modal-content">
            <div class="modal-header clearfix text-left">
              ...
              <h5>Yeni İlçe <span class="semi-bold">Ekle</span></h5>
            </div>
            <div class="modal-body" style="overflow:hidden;">
              <form role="form" class="form-group" id="appendNewCountyData">
              <?php 
              foreach ($formFields as $key => $field) {
                ...
                if($field["type"]!="select"){
                  ...
                  echo '<div class="form-group form-group-default form-group-default-select2 required">
                        <label class="">'.$field["name"].'</label>
                          <select class="full-width '.$field["class"].' '.$field["disabled"].' required" name="'.$field["id"].'" data-msg="'.$message.'" data-placeholder="İlçe seçin" data-init-plugin="select2">
                ...
                }else
                {
                  echo '<div class="form-group form-group-default form-group-default-select2 required">
                        <label class="">'.$field["name"].'</label>
                          <select class="full-width '.$field["class"].' '.$field["disabled"].' required" name="'.$field["id"].'" data-msg="'.$message.'" '.$field["disabled"].' data-placeholder="İlçe seçin" data-init-plugin="select2">
                          <option value="0">Lütfen seçin</option>
                          </select>
                        </div>';
                }
              ...
              ?>
                <button type="submit" class="btn btn-primary addNewCountyButton">Yeni İlçe Ekle</button>
    ...
    <div class="modal fade slide-right" id="modalSlideLeft" tabindex="-1" role="dialog" aria-hidden="true">
      ...
      <h5 class="text-primary "><span class="semi-bold countyNameinModal"></span> adlı ilçeyi silmek istediğinizden emin misiniz? Lütfen bu işlemi geri alamayacağınızı unutmayın.</h5>
      <br>
      <button type="button" class="btn btn-success btn-block deleteCountyButton">Evet</button>
    ...
    <script type="text/javascript">
      $(function(){
        initTable();
        getDetails(".getCountyDetails","/general/getCountyById","countyId",".appendCountyDataHere");
        $("#appendNewCountyData").validate({
          rules: {
            CityID:{min:1},
        },
        ...
        $("#updateCountyData").validate({
          submitHandler: function(form) {
            updateCounty();
            }
        });
        $("#addNewCountyModal").on("shown.bs.modal",function(){
          setTimeout(function () {$('select').select2();}, 300);
        })
        $("body").on("click",".deleteCountyModal",function(){
          var userId = $(this).parents("tr").attr("id");
          var userName = $(this).parents("tr").find("td:eq(0)").html();
          $(".countyNameinModal").html(userName);
          $(".deleteCountyButton").attr("id",userId);
          $("#modalSlideLeft").modal();
        })
        $("body").on("click",".deleteCountyButton",function(){
          var CountyID = $(this).attr("id");
          $(".deleteModalError").html('Lütfen bekleyiniz...').removeClass("unvisible");
          $.ajax({
            type:"POST",
            url:base_url+"/admin/deleteCounty",
