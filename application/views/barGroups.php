...
<html>
  ...
  <body class="fixed-header ">
        ...
        <div class="content ">
          ...
          <div class="container-fluid container-fixed-lg m-t-10">
            ...
            <div class="row">
              ...
              <button class="btn btn-primary pull-right addNewBarGroup m-b-10" data-toggle="modal" data-target="#addNewBarGroupModal">Yeni Bar Grubu Ekle</button>
              </div>
              <div class="col-md-4 pull-left" style="padding-left: 0px">
                <div class="col-xs-12">
                <input type="text" id="search-table" class="form-control pull-left" placeholder="Bar Grubu Ara">
                </div>
              </div>
            </div>
            <div class="row">
              ...
              <tbody>
                ...
                foreach ($barGroups as $key => $barGroup) {
                  echo '<tr id="'.$barGroup->BarGroupID.'">
                    <td>'.$barGroup->Code.'</td>
                    <td>'.$barGroup->Name.'</td>
                    <td><div class="pull-left"><button class="btn btn-warning getBarGroupDetails btn-xs m-r-10" id="duzenle">Düzenle</button><button class="btn btn-danger deleteBarGroupModal btn-xs">Sil</button></div></td>
                  </tr>';
                }
                ?>
              </tbody>
        ...
        <div class="modal-content-wrapper">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form role="form" id="updateBarGroupData">
                <div class="form-group appendBarGroupDataHere">
                </div>
                <button type="submit" class="btn btn-primary m-t-5 updateBarGroup">Bilgileri Düzenle</button>
    ...
    <div class="modal fade slide-right" id="modalSlideLeft" tabindex="-1" role="dialog" aria-hidden="true">
      ...
      <h5 class="text-primary "><span class="semi-bold BarGroupNameinModal"></span> adlı alkol grubunu silmek istediğinizden emin misiniz? Lütfen bu işlemi geri alamayacağınızı unutmayın.</h5>
      <br>
      <button type="button" class="btn btn-success btn-block deleteBarGroupButton">Evet</button>
    ...
    <script type="text/javascript">
      $(function(){
        initTable();
        getDetails(".getBarGroupDetails","/general/getBarGroupById","BarGroupId",".appendBarGroupDataHere");
        $("#appendNewBarGroupData").validate({
          submitHandler: function(form) {
            addNewBarGroup();
            }
        });
        $("#updateBarGroupData").validate({
          submitHandler: function(form) {
            updateBarGroup();
            }
        });
        $("#addNewBarGroupModal").on("shown.bs.modal",function(){
          setTimeout(function () {$('select').select2();}, 300);
        })
        $("body").on("click",".deleteBarGroupModal",function(){
          var userId = $(this).parents("tr").attr("id");
          var userName = $(this).parents("tr").find("td:eq(1)").html();
          $(".deleteModalError").html('').addClass("unvisible");
          $(".BarGroupNameinModal").html(userName);
          $(".deleteBarGroupButton").attr("id",userId);
          $("#modalSlideLeft").modal();
        })
        $("body").on("click",".deleteBarGroupButton",function(){
          var BarGroupID = $(this).attr("id");
          $(".deleteModalError").html('Lütfen bekleyiniz...').removeClass("unvisible");
          $.ajax({
            type:"POST",
            url:base_url+"/admin/deleteBarGroup",
            data:{BarGroupID:BarGroupID},
            success:function(data){
              $(".deleteModalError").html(data.message).removeClass("unvisible");
              $("table").find("tr#"+BarGroupID).fadeOut(500,function(){
                getBarGroups();
              })
              setTimeout(function(){
                $(".deleteModalError").html('').addClass("unvisible");
