...
<html>
  ...
  <body class="fixed-header ">
        ...
        <div class="content ">
          ...
          <div class="container-fluid container-fixed-lg m-t-10">
            ...
            <div class="row">
              ...
              <button class="btn btn-primary pull-right addNewCity" data-toggle="modal" data-target="#addNewCityModal">Yeni Şehir Ekle</button>
              </div>
              <div class="col-md-4 pull-left" style="padding-left: 0px">
                <div class="col-xs-12">
                <input type="text" id="search-table" class="form-control pull-left" placeholder="Şehir Ara">
                </div>
              </div>
            </div>
            <div class="row">
              ...
              <tbody>
                ...
                foreach ($cities as $key => $city) {
                  echo '<tr id="'.$city->CityID.'">
                    <td>'.$city->CityName.'</td>
                    <td>'.$city->PlateNo.'</td>
                    <td>'.$city->PhoneCode.'</td>
                    <td>'.$city->CountryName.'</td>
                    <td><div class="pull-left"><button class="btn btn-warning getCityDetails btn-xs m-r-10" id="duzenle">Düzenle</button><button class="btn btn-danger deleteCityModal btn-xs">Sil</button></div></td>
                  </tr>';
                }
                ?>
              </tbody>
        ...
        <div class="modal-content-wrapper">
          <div class="modal-content">
            <div class="modal-header clearfix text-left">
              ...
              <h5>Şehir <span class="semi-bold">Bilgileri</span></h5>
              <p class="p-b-10">Aşağıda şehir ile ilgili bilgileri bulabilirsiniz.</p>
            </div>
            <div class="modal-body">
              <form role="form" id="updateCityData">
                <div class="form-group appendCityDataHere">
                </div>
                <button type="submit" class="btn btn-primary m-t-5 updateCity">Bilgileri Düzenle</button>
    ...
    <div class="modal fade stick-up disable-scroll" id="addNewCityModal" role="dialog" aria-hidden="false">
      <div class="modal-dialog ">
        <div class="modal-content-wrapper">
          <div class="modal-content">
            <div class="modal-header clearfix text-left">
              ...
              <h5>Yeni Şehir <span class="semi-bold">Ekle</span></h5>
            </div>
            <div class="modal-body" style="overflow:hidden;">
              <form role="form" class="form-group" id="appendNewCityData">
              <?php 
              foreach ($formFields as $key => $field) {
                ...
                }else
                {
                  echo '<div class="form-group form-group-default form-group-default-select2 required">
                        <label class="">'.$field["name"].'</label>
                          <select class="full-width '.$field["class"].' '.$field["disabled"].' required" name="'.$field["id"].'" data-msg="'.$message.'" '.$field["disabled"].' data-placeholder="Şehir seçin" data-init-plugin="select2">
                          <option value="0">Lütfen seçin</option>
                          </select>
                        </div>';
                }
              ...
              ?>
                <button type="submit" class="btn btn-primary addNewCityButton">Yeni Şehir Ekle</button>
    ...
    <div class="modal fade slide-right" id="modalSlideLeft" tabindex="-1" role="dialog" aria-hidden="true">
      ...
      <h5 class="text-primary "><span class="semi-bold cityNameinModal"></span> adlı şehri silmek istediğinizden emin misiniz? Lütfen bu işlemi geri alamayacağınızı unutmayın.</h5>
      <br>
      <button type="button" class="btn btn-success btn-block deleteCityButton">Evet</button>
    ...
    <script type="text/javascript">
      $(function(){
        initTable();
        getDetails(".getCityDetails","/general/getCityById","cityId",".appendCityDataHere");
        $("#appendNewCityData").validate({
          rules: {
            CountryID:{min:1},
        },
        ...
        $("#addNewCityModal").on("shown.bs.modal",function(){
          setTimeout(function () {$('select').select2();}, 300);
        })
        $("body").on("click",".deleteCityModal",function(){
          var userId = $(this).parents("tr").attr("id");
          var userName = $(this).parents("tr").find("td:eq(0)").html();
          $(".cityNameinModal").html(userName);
          $(".deleteCityButton").attr("id",userId);
          $("#modalSlideLeft").modal();
        })
        $("body").on("click",".deleteCityButton",function(){
